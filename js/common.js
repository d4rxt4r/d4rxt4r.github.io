(function() {
    var e = document.querySelector(".header h2");
    function i(e) {
        var i;
        for (i = e.length; i--; ) {
            var t = Math.floor((i + 1) * Math.random())
              , r = e[i];
            e[i] = e[t],
            e[t] = r
        }
    ...
    function t() {
        ...
        n = n + " " + a[a.length - 1],
        a.length -= 1,
        n.replace(/\w\S*/g, function(e) {
            return e.charAt(0).toUpperCase() + e.substr(1).toLowerCase()
        })
    }
    document.querySelector("#rebuzz").addEventListener("click", ()=>{
        e.innerText = t()
    }
    ),
    e.innerText = t()
